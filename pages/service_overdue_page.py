...
class ServiceOverduePage(ReportsNavigationPage):
    ...
    DETAIL_SEARCH_LABELS = (
        "Service Overdue-recordSearchBox",
        "recordSearchBox",
    )
    ...
    def open(self):
        ...
        return self.open_service_overdue(uiform)
    ...
    def select_month(self, uiform, month):
        ...
        result = select_dropdown(uiform, "Month", month)
        if result:
    ...
    def select_overdue_by(self, uiform, option):
        if not uiform:
            return None

        if self.page_filter_state.is_filter_set("overdue_by", option):
            return uiform

        result = select_dropdown(uiform, "Overdue by", option)
        if result:
            self.page_filter_state.set_filter("overdue_by", option)
        return result
    ...
    def select_maintenance_included(self, uiform, option):
        if not uiform:
            return None

        if self.page_filter_state.is_filter_set("maintenance_included", option):
            return uiform

        result = select_dropdown(uiform, "Maintenance Included", option)
        if result:
            self.page_filter_state.set_filter("maintenance_included", option)
        return result
    ...
    def open_service_overdue_card(self, uiform, label):
        if not uiform:
            return None
    ...
    def open_total_overdue_now(self, uiform):
        return self.open_service_overdue_card(uiform, self.CARD_TEXTS[0])

    def open_maximum_days_overdue(self, uiform):
        return self.open_service_overdue_card(uiform, self.CARD_TEXTS[1])

    def open_maximum_kms_overdue(self, uiform):
        return self.open_service_overdue_card(uiform, self.CARD_TEXTS[2])